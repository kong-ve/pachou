...
import { get_notice } from "../../api/config/notice";
import { getAdminId,removeAdminId } from "../../utils/auth";
import { userInfo,logout } from "../../api/auth/login";
...
export default {
    ...
    methods: {
        getNotice(){
            get_notice().then(response=>{
                for(var i = 0;i<response.item;i++){
                    if(response.item[i].state == 0){
                        this.notice_msg++;
                    }
                }
                if(this.notice_msg>0){
                    this.$notify.info({
                    title: '消息',
                    message: '您有'+this.notice_msg+'条未读消息'
                    });
                }
            })
        },
        ...
        userInfo(){
            ...
            formdata['id'] = adminId;
            userInfo(formdata).then(response=>{
                if(response.state != 1){
                    this.$message.error(response.message);
                    return
                }
                if(response.data){
                    this.username = response.data[0].emp_name;
                }
    ...
    created() {
        this.getBreadcrumb();
        // this.userInfo();
        setInterval(this.getNotice(),60000);
    },
    watch: {
        $route() {
            this.getBreadcrumb();
